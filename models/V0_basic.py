...
class BasicPipeline:
  ...
  def __init__(self, query, llm, vectorstore, k=7):
    self.query = query
    self.llm = llm
  ...
  def generate_answer(self):
    basic_retriever = self.vectorstore.as_retriever(search_type="similarity",  search_kwargs={"k": self.k})
    docs = basic_retriever.invoke(self.query)
